...
async function createJSTempFile(taskID, languageID, userCode) {
  // достать test_cases
  ...
  // вернуть файл
  const filePath = path.normalize(path.join(__dirname, '../../TempFiles', `tempCode-${Date.now()}.js`));
  fs.writeFileSync(filePath, codeTemplate);
  return filePath;
}

module.exports = createJSTempFile;