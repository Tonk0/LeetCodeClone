...
async function createPythonTempFile(taskID, languageID, userCode) {
  // достать test_cases
  ...
  // вернуть файл
  const filePath = path.normalize(path.join(__dirname, '../../TempFiles', `tempCode-${Date.now()}.py`));
  fs.writeFileSync(filePath, codeTemplate);
  return filePath;
}

module.exports = createPythonTempFile;